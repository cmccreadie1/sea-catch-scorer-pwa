  ...
  e.respondWith(
    caches.match(e.request).then((response) => {
      ...
      return fetch(e.request).then((fetchResponse) => {
        // 3. Save a copy of this new file to the cache for next time
        return caches.open(CACHE_NAME).then((cache) => {
          // Only cache valid GET requests (prevents errors with API fetches like the Weather one)
      ...
      // Failsafe: If offline and the file isn't cached, fail silently rather than crashing
